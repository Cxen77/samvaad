  ...
  useEffect(() => {
    if (callState === 'CONNECTED' || callState === 'OUTGOING' || callState === 'RINGING') {
      if (localVideoRef.current && localStreamRef.current) {
        ...
        console.log('[DirectCall] Local video element stream attached');
      }
      if (remoteVideoRef.current && remoteStreamRef.current) {
        remoteVideoRef.current.srcObject = remoteStreamRef.current;
        console.log('[DirectCall] Remote video element stream attached');
      }
      // CRITICAL FOR AUDIO-ONLY CALLS: Attach remote stream to HTMLAudioElement
      if (remoteAudioRef.current && remoteStreamRef.current) {
        remoteAudioRef.current.srcObject = remoteStreamRef.current;
        console.log('[DirectCall] Remote audio element created and stream attached');
        remoteAudioRef.current.play()
          .then(() => {
            console.log('[DirectCall] Remote audio playback started');
          })
          .catch((err) => {
            console.warn('[DirectCall] Remote audio playback waiting for user gesture:', err.message);
          });
      }
    }
      ...
      <div className="fixed inset-0 z-[9999] bg-slate-950 flex flex-col font-sans overflow-hidden text-white">
        ...
        {showSecurityModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-[10000] flex items-center justify-center p-4" onClick={() => setShowSecurityModal(false)}>
            <div 
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-sm w-full p-5 space-y-4 shadow-2xl text-slate-900 dark:text-white"
            ...
            >
              ...
              <div className="space-y-2 text-xs text-slate-700 dark:text-slate-300">
                ...
                <p className="flex items-center justify-between border-t border-slate-200 dark:border-slate-800 pt-2 text-[10px]">
                  ...
                  <span className="font-mono text-slate-500 dark:text-slate-400">{callData?.callId}</span>
                </p>
              </div>
            </div>
          </div>
